"""
backtest_combo3.py — Baseline vs Combo 3 fixes validés
=======================================================
Périodes : 2022 Bear + 2023 Recovery — Binance US 5min
Fill rate : 65% (seed=42)
...
CONFIGS = {
    "Baseline (actuel)":      {"rr_high": False, "p3b_n": 8, "wilder": False},
    "Combo 3 fixes (1+3+5)":  {"rr_high": True,  "p3b_n": 4, "wilder": True},
}
...
def _rsi_sma(c, p=14):
    ...
    d = np.diff(c.astype(float)); g = np.where(d>0,d,0.); l = np.where(d<0,-d,0.)
...
# ── BACKTEST (numpy-indexed, fast) ────────────────────────────────────────────

def run_period(sym_data, cfg):
    ...
    for zk, p in open_pos.items():
        ...
        trades.append({"sym":p["sym"],"entry":p["entry"],"exit":cl,
                       ...
                       "pct":round((cl-p["entry"])/p["entry"]*100,3)})
...
def print_stats(name, s):
    ...
    print(f"  Exits     : stops={s['stops']}  targets={s['tgts']}  timeout/end={s['tos']}")
    print(f"  By symbol : ETH {s['eth_n']} trades ${s['eth_pnl']:>+.0f}"
          f"  |  SOL {s['sol_n']} trades ${s['sol_pnl']:>+.0f}")
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("  BACKTEST COMBO 3 FIXES — ETH+SOL — Binance US 5m")
    print("  Fixes : R:R from zone_high | Pass3b 4b | RSI Wilder")
    print("="*60)

    all_results = {}   # period → config → stats
    ...
    for period_name, (start, end) in PERIODS.items():
        ...
        for cname, cfg in CONFIGS.items():
            trades, capital, days = run_period(sym_data, cfg)
            s = compute_stats(trades, capital, days)
            all_results[period_name][cname] = s
            print_stats(cname, s)
    ...
    print(f"{'='*60}")
    print(f"\n  {'Métrique':<18}  {'Baseline 2022':>13}  {'Combo 2022':>12}  "
          f"{'Baseline 2023':>13}  {'Combo 2023':>12}")
    print(f"  {'─'*18}  {'─'*13}  {'─'*12}  {'─'*13}  {'─'*12}")
    metrics = [
        ("N trades",  "n",     "d"),
        ("Win rate",  "wr",    "p"),
        ("Profit F.", "pf",    "f"),
    ...
    for label, key, fmt in metrics:
        ...
        for pname in PERIODS:
            for cname in CONFIGS:
                s = all_results[pname][cname]
                v = s[key] if s else 0
                if   fmt=="$": vals.append(f"${v:>+9.0f}")
                elif fmt=="p": vals.append(f"{v:>11.1f}%")
                elif fmt=="a": vals.append(f"{v:>11.1f}%")
                elif fmt=="f": vals.append(f"{v:>13.2f}")
                else:          vals.append(f"{v:>13d}")
        print(f"  {label:<18}  {'  '.join(vals)}")

    # ── Delta summary ─────────────────────────────────────────────────────────
    print(f"\n  {'─'*60}")
    print("  DELTA (Combo − Baseline)")
    print(f"  {'─'*60}")
    for pname in PERIODS:
        b = all_results[pname]["Baseline (actuel)"]
        c = all_results[pname]["Combo 3 fixes (1+3+5)"]
        if b and c:
            dpnl = c["total"] - b["total"]
            dwr  = c["wr"]    - b["wr"]
            dpf  = c["pf"]    - b["pf"]
            dmdd = c["mdd"]   - b["mdd"]
            sign = "+" if dpnl >= 0 else ""
            print(f"  {pname:<16}: PnL {sign}${dpnl:>+.0f}  WR {dwr:>+.1f}%  "
                  f"PF {dpf:>+.2f}  MDD {dmdd:>+.1f}%")
    print()
